...
export interface ApiRequest { action: 'login' | 'dashboard' | 'createSession' | 'migrateSession' | 'closeSession' | 'deleteSession' | 'session' | 'scan' | 'logout' | 'adminAccounts' | 'createAdminAccount' | 'updateAdminAccount' | 'removeAdminAccount' | 'students' | 'createStudent' | 'updateStudent' | 'setStudentActive'; payload?: Record<string, unknown>; token?: string; }
...
export interface BackendAdapter { login(payload: Record<string, unknown>, token: string): Promise<unknown>; authorizeAdmin(adminId: string, sessionId: string): Promise<unknown>; dashboard(token: string): Promise<unknown>; createSession(input: CreateSessionRpcInput, token: string): Promise<unknown>; closeSession(sessionId: string, token: string): Promise<unknown>; deleteSession(sessionId: string, token: string): Promise<unknown>; session(sessionId: string, qrTokenHash: string): Promise<unknown>; scan(input: ScanRpcInput): Promise<unknown>; logout(token: string): Promise<unknown>; consumeRateLimit(scope: RateLimitScope, subjectHash: string, limit: number, windowSeconds: number): Promise<RateLimitDecision>; students(): Promise<unknown>; createStudent(studentId: string, name: string): Promise<unknown>; updateStudent(studentId: string, name: string): Promise<unknown>; setStudentActive(studentId: string, active: boolean): Promise<unknown>; adminAccounts(): Promise<unknown>; createAdminAccount(email: string, password: string, username: string | null, role: string): Promise<unknown>; updateAdminAccount(adminId: string, username: string | null, role: string, active: boolean, password?: string): Promise<unknown>; removeAdminAccount(adminId: string): Promise<unknown>; }
export interface EdgeConfig { allowedOrigin: string; supabaseUrl: string; supabaseAnonKey: string; supabaseServiceRoleKey: string; authIssuer: string; authAudience: string; authJwksUrl: string; adminLoginEmail: string; adminAllowedRoles: string[]; sessionTimeOffset: string; qrKeyring: QrKeyring; rateLimitHmacSecret: string; migrationMode: boolean; }
